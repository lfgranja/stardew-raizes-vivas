...
#pragma warning restore S1144
        ...
        private static T CreateInstanceWithFallbackFixed<T>()
            where T : class
        {
        ...
        private static T? TryCreateInstanceWithActivator<T>(ref Exception? lastError)
            where T : class
        {
            try
            {
        ...
        private static bool CanCreateInstanceFromConstructor(ConstructorInfo constructor)
        {
            ...
            return !parameters.Any(p =>
                !p.IsOptional && !p.HasDefaultValue && !p.ParameterType.IsValueType
            );
        }

        private static T? TryCreateInstanceFromConstructor<T>(
        ...
        public void ParameterReflection_WhenParameterIsOptional_HasDefaultValue()
        {
            ...
            var constructor = typeof(TypeWithOptionalParametersAndDefaults).GetConstructor(
