...
namespace LivingRoots.Tests
{
    public class ModDataServiceTests
    {
        ...
        public ModDataServiceTests()
        {
            ...
            SetupSanitizeFileNameMock();
            SetupValidatePathMock();
        }

        private void SetupSanitizeFileNameMock()
        {
            ...
            _mockModLogic
                ...
                {
                    return SanitizeFileNameForTest(input);
                });
        }

        private static string SanitizeFileNameForTest(string input)
        {
            ...
            if (input == "file....name")
                return "file.name";
            if (IsTrimmedTestKey(input))
                return "test_key";
            if (ShouldThrowEmptyResultException(input))
                throw new ArgumentException(
                    "Filename sanitizes to an empty string.",
                    nameof(input)
                );
            if (ShouldThrowInvalidPathComponentException(input))
                throw new ArgumentException(
                    $"Filename sanitizes to invalid path component '{input}'.",
                    nameof(input)
                );
            if (ContainsInvalidCharacters(input))
                return ReplaceInvalidCharacters(input);
            if (ContainsMultipleDots(input))
                return ReplaceMultipleDots(input);
            return input;
        }

        private static bool IsTrimmedTestKey(string input)
        {
            return input == "  test_key " || input == "test_key " || input == " test_key";
        }

        private static bool ShouldThrowEmptyResultException(string input)
        {
            return input == "<>:\"|?*" || input == "........." || input == "___" || input == "   ";
        }

        private static bool ShouldThrowInvalidPathComponentException(string input)
        {
            return input == "." || input == "..";
        }
        ...
        private static bool ContainsMultipleDots(string input)
        {
            return input.Contains("....");
        }

        private static string ReplaceMultipleDots(string input)
        {
            return input.Replace("....", ".").Replace("...", ".");
        }

        private void SetupValidatePathMock()
        {
        ...
        public void SaveData_WithValidData_CallsWriteJsonFile()
        {
            ...
            // Assert
            _mockDataHelper.Verify(
                x => x.WriteJsonFile("data/test_key.json", testData),
                Times.Once
            );
        }
        ...
        public void SaveData_WithNullData_ThrowsArgumentNullException()
        {
            ...
            var exception = Assert.Throws<ArgumentNullException>(() =>
                service.SaveData<object>(null!, "test_key")
            );
        ...
        public void LoadData_WithValidKey_CallsReadJsonFile()
        {
            ...
            _mockDataHelper
                .Setup(x => x.ReadJsonFile<object>("data/test_key.json"))
        ...
        public void RemoveData_WithExistingData_RemovesData()
        {
            ...
            // Assert
            _mockDataHelper.Verify(
                x => x.WriteJsonFile<object>("data/test_key.json", null),
                Times.Once
            );
        }
        ...
        public void GetFilePath_WithInvalidChars_Sanitizes()
        {
            ...
            _mockDataHelper.Verify(
                x => x.WriteJsonFile("data/test/key/with_invalid_chars.json", testData),
                Times.Once
            );
        }
        ...
        public void GetFilePath_WithPathTraversal_ThrowsArgumentException()
        {
            ...
            _mockModLogic
                .Setup(x => x.ValidatePath(It.Is<string>(s => s.Contains("../../../"))))
                .Throws<ArgumentException>();
            var service = new ModDataService(
                _mockHelper.Object,
                _mockMonitor.Object,
            ...
            Assert.Throws<ArgumentException>(() =>
                service.SaveData(testData, "../../../etc/passwd")
            );
        }
        ...
        public void LoadData_WithDirectoryNotFoundException_ReturnsNull()
        {
            ...
            var directoryNotFoundException = new System.IO.DirectoryNotFoundException(
        ...
        public void LoadData_WithUnauthorizedAccessException_ReturnsNull()
        {
            ...
            var unauthorizedAccessException = new System.UnauthorizedAccessException(
                "Access denied"
            );
            _mockDataHelper
                .Setup(x => x.ReadJsonFile<object>("data/test_key.json"))
        ...
        public void LoadData_WithFileNotFoundException_LogsSanitizedKey()
        {
            ...
            _mockMonitor.Verify(
                x => x.Log(It.Is<string>(msg => msg.Contains("test_key")), It.IsAny<LogLevel>()),
                Times.Once
            );
        }
        ...
        public void LoadData_WithMissingFile_ReturnsNullAndLogsTrace()
        {
            ...
            var serviceWithDir = new ModDataService(
                mockHelperWithDir.Object,
                mockMonitor.Object,
                mockModLogic.Object
            );
            ...
            mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains("test_key") && msg.Contains("No valid data found")
                        ),
                        LogLevel.Trace
                    ),
                Times.Once
        ...
        public void LoadData_WithCorruptFile_ReturnsNullAndLogsWarn()
        {
            ...
            _mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains("test_key") && msg.Contains("File contains no valid data")
                        ),
                        LogLevel.Warn
                    ),
                Times.Once
        ...
        public void DataExists_WithFileNotFoundException_LogsSanitizedKey()
        {
            ...
            _mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains("test_key") && !msg.Contains("test_key.json")
                        ),
                        LogLevel.Trace
                    ),
                Times.Once
        ...
        public void SaveData_WithIOException_LogsSanitizedPath()
        {
            ...
            var ioException = new System.IO.IOException("IO error");
            _mockDataHelper
                .Setup(x => x.WriteJsonFile("data/test_key.json", testData))
                .Throws(ioException);
            ...
            _mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains("IOException occurred while saving data")
                        ),
                        LogLevel.Warn
                    ),
                Times.Once
        ...
        public void SaveData_WithDangerousPathKey_DoesNotLogFullFilePath()
        {
            ...
            _mockModLogic
                .Setup(x =>
                    x.SanitizeFileName(It.Is<string>(s => s != "dangerous" && s != "path.exe"))
                )
                .Returns<string>(s => s);
            ...
            _mockDataHelper
                .Setup(x => x.WriteJsonFile(It.IsAny<string>(), testData))
        ...
        public void LoadData_WithValidationExceptionInGetValidatedAndSanitizedKey_ReturnsNull()
        {
            ...
            _mockModLogic
                ...
                .Throws(new ArgumentException("Invalid path provided"));
            var service = new ModDataService(
                _mockHelper.Object,
                _mockMonitor.Object,
                _mockModLogic.Object
        ...
        public void DataExists_WithCorruptJsonFile_ReturnsFalse()
        {
            ...
            _mockDataHelper
                .Setup(x => x.ReadJsonFile<object>("data/corrupt_key.json"))
                .Throws(jsonException);
            ...
            _mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains(
                                "JSON parsing error occurred while checking data existence for key 'corrupt_key'"
                            )
                        ),
                        LogLevel.Warn
                    ),
        ...
        public void DataExists_WithValidFile_ReturnsTrue()
        {
            ...
            _mockDataHelper
                .Setup(x => x.ReadJsonFile<object>("data/valid_key.json"))
                .Returns(testData);
        ...
        public void DataExists_WithNonExistentFile_ReturnsFalse()
        {
            ...
            _mockDataHelper
                .Setup(x => x.ReadJsonFile<object>("data/nonexistent_key.json"))
                .Throws(fileNotFoundException);
            ...
            _mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains(
                                "File not found while checking data existence for key 'nonexistent_key'"
                            )
                        ),
                        LogLevel.Trace
                    ),
        ...
        public void LoadData_WithNullHelper_ReturnsNull()
        {
            ...
            var helperField = typeof(ModDataService).GetField(
                "_helper",
                BindingFlags.NonPublic | BindingFlags.Instance
            );
        ...
        public void LoadData_WithNullDataHelper_ReturnsNull()
        {
            ...
            mockMonitor.Verify(
                x =>
                    x.Log(
                        It.Is<string>(msg =>
                            msg.Contains("Helper.Data is null in LoadData method")
                        ),
                        LogLevel.Error
                    ),
                Times.Once
        ...
        public void DataExists_WithNullHelper_ReturnsFalse()
        {
            ...
            // Replace the helper in the service using reflection
            var serviceField = typeof(ModDataService).GetField(
                "_helper",
                BindingFlags.NonPublic | BindingFlags.Instance
            );
        ...
        public void GetFilePath_WithCrossPlatformPathSeparators_ConstructsCorrectly()
        {
            ...
            service.SaveData(
                testData,
                "test/key" + Path.DirectorySeparatorChar + "with:invalid|chars"
            );
            ...
            _mockDataHelper.Verify(
                x =>
                    x.WriteJsonFile(
                        Path.Combine("data", "test", "key", "with_invalid_chars.json")
                            .Replace('\\', '/'),
                        testData
                    ),
                Times.Once
