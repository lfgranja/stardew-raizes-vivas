...
namespace LivingRoots.Tests
{
    public class ExtensionDetectionTest
    {
        ...
        public void TestExtensionDetectionWithUnicodeThroughPublicApi()
        {
            ...
            mockUnicodeService.Setup(x => x.Normalize("test\u0301.txt")).Returns("test\u0301.txt"); // Using the original form for this test

            // Setup reserved name handler to return input unchanged
            mockReservedService.Setup(x => x.Handle(It.IsAny<string>())).Returns<string>(s => s);

            var service = new FileNameSanitizationService(
                mockUnicodeService.Object,
                mockReservedService.Object
            );
        ...
        public void TestFindExtensionStartIndexScenariosThroughPublicApi()
        {
            ...
            mockUnicodeService.Setup(x => x.Normalize(It.IsAny<string>())).Returns<string>(s => s);
