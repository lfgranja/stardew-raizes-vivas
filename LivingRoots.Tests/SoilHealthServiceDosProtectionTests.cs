...
namespace LivingRoots.Tests
{
    public class SoilHealthServiceDosProtectionTests
    {
        ...
        public void LoadData_WithPerLocationTileLimitExceeded_AbortsEntireLoadIncludingValidLocations()
        {
            ...
            for (var i = 0; i < excessTiles; i++)
            {
                Assert.InRange(service.GetSoilHealth("ExceedingLocation", new Vector2(i, 0)), 20f, 100f); // Returns value in [20, 100] when cache is cleared
            }

            // The valid location should NOT be processed since the entire load is aborted
            Assert.InRange(service.GetSoilHealth("ValidLocation", new Vector2(100, 100)), 20f, 100f); // Returns value in [20, 100] when cache is cleared
        }
        ...
        public void LoadData_WhenOneLocationExceedsLimit_AbortsEntireLoad()
        {
            ...
            for (var i = 0; i < atLimitTiles; i++)
            {
                Assert.InRange(service.GetSoilHealth("AtLimitLocation", new Vector2(i, 0)), 20f, 100f); // Returns value in [20, 100] when cache is cleared
            }
