namespace LivingRoots.Domain
{
        ...
        : IReservedNameHandler
    {
        ...
        public string? Handle(string? filename)
        {
            ...
            {
                return HandleUncPath(normalizedInput, filename);
            }
            else
            {
                return HandleRegularPath(normalizedInput, filename);
            }
        }

        private static string? HandleUncPath(string normalizedInput, string originalFilename)
        {
            ...
            var fileName = ExtractFileNameFromUncPath(normalizedInput);
            ...
            if (string.IsNullOrEmpty(fileName))
            {
                return HandleUncDirectoryPath(normalizedInput, originalFilename);
            }
        ...
        )
        {
            ...
            var processedFileName = ProcessFileNameInternal(fileName);
            ...
            if (!string.IsNullOrEmpty(uncDirectoryPath))
            {
                ...
                return uncDirectoryPath + "\\" + processedFileName;
            }

            return processedFileName;
        }

        private static string? HandleRegularPath(string normalizedInput, string originalFilename)
        {
            ...
            if (string.IsNullOrEmpty(fileName))
            {
                return HandleRegularDirectoryPath(normalizedInput, originalFilename);
            }
        ...
        )
        {
            ...
            if (!string.IsNullOrEmpty(regDirectoryPath))
            {
                return Path.Combine(regDirectoryPath, processedFileName);
            }
        ...
        private static string ExtractLastPathComponent(string path, char separator)
        {
            ...
            else if (lastSeparatorIndex == path.Length - 1)
            {
                ...
                for (var i = lastSeparatorIndex - 1; i >= 0; i--)
                {
                    if (path[i] == separator)
                    {
        ...
        )
        {
            ...
            var modifiedComponent = ProcessFileNameInternal(lastComponent);
            ...
            else
            {
                ...
                var dirBeforeLastComponent = path.Substring(0, lastSeparatorIndex + 1);
        ...
        {
            var uncPrefixEnd = FindUncPrefixEnd(uncPath);
            if (uncPrefixEnd == -1)
                return Path.GetFileName(uncPath);

            var separator = uncPath[0];
            var serverEnd = FindServerEnd(uncPath, uncPrefixEnd, separator);
            if (serverEnd == -1)
                return Path.GetFileName(uncPath);

            var shareEnd = FindShareEnd(uncPath, serverEnd, separator);
            if (shareEnd == -1)
                return Path.GetFileName(uncPath);

            var fileNameStart = shareEnd + 1;
        ...
        private static int FindServerEnd(string uncPath, int uncPrefixEnd, char separator)
        {
            for (var i = uncPrefixEnd; i < uncPath.Length; i++)
            {
        ...
        private static int FindShareEnd(string uncPath, int serverEnd, char separator)
        {
            for (var i = serverEnd + 1; i < uncPath.Length; i++)
            {
                if (uncPath[i] == separator)
                    return i;
            }
        ...
        private static string ProcessFileNameInternal(string filename)
        {
            ...
            var firstExtensionIndex = FindFirstExtensionIndex(filename);
            ...
            if (IsReservedName(baseName))
            {
                ...
                var sanitizedExtension = SanitizeTrailingInsignificantChars(extensionPart);
                ...
                if (baseName.Trim('.', ' ', '\t') != baseName)
                {
                    ...
                    var significantPart = baseName.TrimEnd('.', ' ', '\t');
            ...
            if (IsReservedName(baseNameTrimmed))
            {
                ...
                var startIndex = FindStartIndexAfterInsignificantChars(baseName);
                var endIndex = FindEndIndexBeforeInsignificantChars(baseName);

                // Extract the leading insignificant characters, the core name, and trailing insignificant characters
                var leadingChars = startIndex > 0 ? baseName.Substring(0, startIndex) : "";
                var coreName = baseName.Substring(startIndex, endIndex - startIndex + 1);

                // Add underscore to the core reserved name
                var modifiedCore = coreName + "_";
            ...
            var trimmedForInsignificantCheck = baseName.Trim('.', ' ', '\t');
        ...
        private static int FindFirstExtensionIndex(string filename)
        {
            ...
            if (filename.StartsWith('.'))
            {
                ...
                var firstDotAfterInitial = filename.IndexOf('.', 1);
            ...
            for (var i = 0; i < filename.Length; i++)
            {
                if (filename[i] == '.')
                {
                    ...
                    if (IsReservedNameWithExtension(potentialBaseName))
                    {
                        return i;
                    }
                }
        ...
        private static bool IsReservedNameWithExtension(string potentialBaseName)
        {
            ...
            foreach (var reservedNameLength in reservedNameLengths)
            {
                if (reservedNameLength < potentialBaseName.Length)
                {
                    ...
                    var nextChar = potentialBaseName[reservedNameLength];
        ...
        private static bool IsReservedName(string name)
        {
            ...
            foreach (var reservedNameLength in reservedNameLengths)
            {
                ...
                {
                    // Check if what follows is not alphanumeric (meaning it's not just a longer non-reserved name)
                    var nextChar = name[reservedNameLength];
                    if (!char.IsLetterOrDigit(nextChar))
                    {
